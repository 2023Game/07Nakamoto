...
void CMesh::AnimateVertex(CMatrix* mat) {
	...
	for (size_t i = 0; i < mSkinWeights.size(); i++) {
		...
		//フレーム合成行列にオフセット行列を合成
		CMatrix mSkinningMarix = mSkinWeights[i]->mOffset * mat[frameIndex];
		//頂点数分繰り返し
		for (int j = 0; j < mSkinWeights[i]->mIndexNum; j++) {
			...
			//頂点と法線を更新する
			mpAnimateVertex[index] += mpVertex[index] * mSkinningMarix * weight;
			mpAnimateNormal[index] += mpNormal[index] * mSkinningMarix * weight;
		}
	}
	//法線を正規化する
...
void CMesh::SetSkinWeightFrameIndex(CModelX* modl)
{
	//スキンウェイト分繰り返し
	for (size_t i = 0; i < mSkinWeights.size(); i++) {
		//フレーム名のフレームを取得
		CModelXFrame* frame = modl->FindFrame(mSkinWeights[i]->mpFrameName);
		//フレーム番号の設定をする
